                ...
                <div key={deal.id} className={`deal-card${revealedDeals.has(deal.id) ? ' revealed' : ''}`}>
                  {revealedDeals.has(deal.id) ? (
                    ...
                    <div className="deal-revealed-card">
                      <div className="deal-revealed-top">
                        <button
                          type="button"
                      ...
                      {deal.image && (
                        <div className="deal-revealed-image">
                          <Image src={mediaUrl(deal.image)!} alt={deal.title} width={300} height={200} quality={75} style={{ objectFit: 'contain', width: '100%', height: '100%' }} />
                        </div>
                      )}
                      <h4>{deal.title}</h4>
                      ...
                      <a
                        href={`https://wa.me/60102323620?text=${encodeURIComponent(`Hi! I'm interested in: ${deal.title}${deal.brand ? ` by ${deal.brand}` : ''} — Deal Price: RM${deal.dealPrice?.toLocaleString() || '?'} at ${exhibition.title}`)}`}
                        target="_blank"
